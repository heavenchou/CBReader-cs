...
namespace CBReader
{
	class CNextLine
	{
		public bool InNextLine = false; // 判斷是否在隔行的 <tt> 中
		public bool IsOutput = true;        // 判斷可以呈現了嗎? 如果有資料在 </tt> 後面, 此時可能還不能輸出, 要一併記錄在第二行裡面
		public int TCount = 0;             // 第幾個 <t> 標記, 第一個在本行, 第二個在下一行

		public string ThisLine = "";        // 這一行的內容
		public string NextLine = "";		// 下一行的內容
