...
namespace Monster
{
    ...
    */
    public class CWordData
    {
		...
		IntPtr ipBuffer1;		// 產生 WordPos 空間的指標

		unsafe int* pBuffer;
		unsafe int* Buffer1;      // 每一個字所佔的大量空間, 用來包含底下的東西 (新版已解壓縮的)
        ...
        unsafe public void Initial(int iOffset, int iBufferSize)
		{
			...
			fixed(byte* tmpB = Buffertmp) {
				...
				ipBuffer1 = Marshal.AllocHGlobal(iTotalDataCount * 4); // int 4 個 byte
				Buffer1 = (int *) ipBuffer1.ToPointer();
		...
		unsafe public void ZeroIt()
		{
			...
			fixed (byte* t = BufferTmp) {
				...
				ipBuffer1 = Marshal.AllocHGlobal(5 * 4); // int 4 個 byte
				Buffer1 = (int*) ipBuffer1.ToPointer();
				*Buffer1 = 0;

				for (int i = 0; i < FileCount; i++) {
					WordCount[i] = 0;           // 每一檔出現字數, 清為 0
					WordPos[i] = Buffer1;         // 所以每一字的位置也是 0
				}
			}
		}
    }
